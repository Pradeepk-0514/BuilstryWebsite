...
function CtaModal({ mode, onClose }) {
  ...
  return (
    <div className="cta-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="cta-modal" role="dialog" aria-modal="true" aria-labelledby="cta-modal-title">
        ...
        {submitted ? <div className="cta-success"><span className="section-kicker">BUILSTRY</span><h2>Thank you.</h2><p>We’ll be in touch shortly.</p>{isBooking && <small>This is a call request, not a calendar confirmation.</small>}<button type="button" className="button button-primary" onClick={onClose}>Close <span>→</span></button></div> : <>
